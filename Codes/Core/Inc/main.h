/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2024 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f1xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define imac_power_Pin GPIO_PIN_13
#define imac_power_GPIO_Port GPIOC
#define Ext_IO3_Pin GPIO_PIN_15
#define Ext_IO3_GPIO_Port GPIOC
#define Ext_IO2_Pin GPIO_PIN_0
#define Ext_IO2_GPIO_Port GPIOA
#define Ext_IO1_Pin GPIO_PIN_1
#define Ext_IO1_GPIO_Port GPIOA
#define Ext_BTN_Pin GPIO_PIN_2
#define Ext_BTN_GPIO_Port GPIOA
#define SR_CP_Pin GPIO_PIN_5
#define SR_CP_GPIO_Port GPIOA
#define SR_OUT_Pin GPIO_PIN_6
#define SR_OUT_GPIO_Port GPIOA
#define SR_PL_Pin GPIO_PIN_7
#define SR_PL_GPIO_Port GPIOA
#define MCU_LEDR_Pin GPIO_PIN_12
#define MCU_LEDR_GPIO_Port GPIOB
#define Coin_Reader_Pin GPIO_PIN_13
#define Coin_Reader_GPIO_Port GPIOB
#define ticket_nuch_Pin GPIO_PIN_14
#define ticket_nuch_GPIO_Port GPIOB
#define ticket_nuch_EXTI_IRQn EXTI15_10_IRQn
#define en_ticket_Pin GPIO_PIN_15
#define en_ticket_GPIO_Port GPIOB
#define USB_PU_Pin GPIO_PIN_10
#define USB_PU_GPIO_Port GPIOA
#define ASK_IN_SIG_Pin GPIO_PIN_15
#define ASK_IN_SIG_GPIO_Port GPIOA

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
